...
import { processCustomerHit, updateCustomerPositions } from '../logic/customerSystem';
import { soundManager } from '../utils/sounds';

/**
 * =========================================================
 * Entity factories (Refactor #3)
 * =========================================================
 */

const createPizzaSlice = (lane: number): PizzaSlice => ({
  id: `pizza-${Date.now()}-${lane}`,
  lane,
  position: GAME_CONFIG.CHEF_X_POSITION,
  speed: ENTITY_SPEEDS.PIZZA,
});

const createPowerUp = (params: {
  id: string;
  lane: number;
  position: number;
  speed: number;
...
const createCustomer = (params: {
  ...
  leaving: false,
  disappointed: false,
  disappointedEmoji: params.disappointedEmoji,
  movingRight: false,
  critic: params.critic,
  badLuckBrian: params.badLuckBrian,
  flipped: params.badLuckBrian,
});

const createBossMinion = (params: {
  id: string;
  lane: number;
  position: number;
}): BossMinion => ({
...
const createDroppedPlate = (params: {
  id: string;
  lane: number;
  position: number;
  startTime: number;
  ...
  hasSlice: params.hasSlice,
});

const makeIdleOvenSoundStates = (): Record<number, OvenSoundState> => ({
  0: 'idle',
  1: 'idle',
  2: 'idle',
  3: 'idle',
...
export const useGameLogic = (gameStarted: boolean = true) => {
  ...
  const [ovenSoundStates, setOvenSoundStates] = useState<Record<number, OvenSoundState>>(
    makeIdleOvenSoundStates(),
  );

  const prevShowStoreRef = useRef(false);

  const addFloatingScore = useCallback(
    (points: number, lane: number, position: number, state: GameState): GameState => {
      const now = Date.now();
      return {
        ...state,
        floatingScores: [
          ...state.floatingScores,
          { id: `score-${now}-${Math.random()}`, points, lane, position, startTime: now },
        ],
      };
    },
    [],
  ...
  /**
   * Refactor #1:
   * Centralized customer award logic (scoring/bank/streak/stats/happy count).
   * NOTE: life-gain rules vary by context, so those remain where they were.
   */
  const awardCustomer = (
    s: GameState,
    customer: Customer,
  ...
  ): { points: number; lane: number; position: number } => {
    const pointsEarned = Math.floor(
      baseScore * dogeMultiplier * getStreakMultiplier(s.stats.currentCustomerStreak),
    );

    s.score += pointsEarned;
    s.bank += SCORING.BASE_BANK_REWARD * dogeMultiplier;

    s.happyCustomers += 1;
    s.stats.customersServed += 1;
    s.stats.currentCustomerStreak += 1;

    if (s.stats.currentCustomerStreak > s.stats.longestCustomerStreak) {
      s.stats.longestCustomerStreak = s.stats.currentCustomerStreak;
    }

    return { points: pointsEarned, lane: customer.lane, position: customer.position };
  };

  const awardPointsOnly = (
    s: GameState,
    lane: number,
    position: number,
    baseScore: number,
    ...
    const pointsEarned = Math.floor(baseScore * dogeMultiplier * streakMultiplier);
    s.score += pointsEarned;
    s.bank += SCORING.BASE_BANK_REWARD * dogeMultiplier;
    return { points: pointsEarned, lane, position };
  };

  const gainLifeFromHappyCustomers = (s: GameState, dogeMultiplier: number) => {
    if (s.happyCustomers % 8 === 0 && s.lives < GAME_CONFIG.MAX_LIVES) {
      const starsToAdd = Math.min(dogeMultiplier, GAME_CONFIG.MAX_LIVES - s.lives);
      s.lives += starsToAdd;
      if (starsToAdd > 0) soundManager.lifeGained();
    }
  };

  const gainCriticLifeAtPosition = (s: GameState, customer: Customer, minPosition: number) => {
    if (customer.critic && customer.position >= minPosition && s.lives < GAME_CONFIG.MAX_LIVES) {
      s.lives += 1;
  ...
  const triggerGameOver = useCallback((state: GameState, now: number): GameState => {
    ...
    setOvenSoundStates(makeIdleOvenSoundStates());
  ...
  /**
   * Refactor #2:
   * Power-up effects table (caught powerups).
   * Keeps exact behaviors; only removes the big if/else chain.
   */
  const applyCaughtPowerUpEffect = useCallback(
    (
      type: PowerUpType,
      state: GameState,
      now: number,
      ctx: {
        hasDoge: boolean;
        dogeMultiplier: number;
        pushScore: (entry: { points: number; lane: number; position: number }) => void;
      },
    ): GameState => {
      const handlers: Record<PowerUpType, (s: GameState) => GameState> = {
        beer: (s) => {
          let livesLost = 0;
          let lastReason: StarLostReason | undefined;

          s.customers = s.customers.map((customer) => {
            if (customer.critic) {
              if (customer.woozy) {
                return {
                  ...customer,
          ...
          if (livesLost > 0) {
            ...
            if (lastReason) s.lastStarLostReason = lastReason;
          }

          if (s.lives === 0) return triggerGameOver(s, now);
        ...
        star: (s) => {
          ...
          s.activePowerUps = [
            ...s.activePowerUps.filter((p) => p.type !== 'star'),
        ...
        doge: (s) => {
          s.activePowerUps = [
            ...s.activePowerUps.filter((p) => p.type !== 'doge'),
        ...
        nyan: (s) => {
          if (!s.nyanSweep?.active) {
            ...
            if (!ctx.hasDoge || s.powerUpAlert?.type !== 'doge') {
        ...
        moltobenny: (s) => {
          const moltoScore = SCORING.MOLTOBENNY_POINTS * ctx.dogeMultiplier;
          const moltoMoney = SCORING.MOLTOBENNY_CASH * ctx.dogeMultiplier;

          s.score += moltoScore;
          s.bank += moltoMoney;

          ctx.pushScore({ points: moltoScore, lane: s.chefLane, position: GAME_CONFIG.CHEF_X_POSITION });
          return s;
        },

        honey: (s) => {
          s.activePowerUps = [
            ...s.activePowerUps.filter((p) => p.type !== 'honey'),
            { type: 'honey', endTime: now + POWERUPS.DURATION },
          ];
        ...
        'ice-cream': (s) => {
          s.activePowerUps = [
            ...s.activePowerUps.filter((p) => p.type !== 'ice-cream'),
            { type: 'ice-cream', endTime: now + POWERUPS.DURATION },
          ];
  ...
  const spawnCustomer = useCallback(() => {
    const now = Date.now();
    const spawnDelay =
      SPAWN_RATES.CUSTOMER_MIN_INTERVAL_BASE - gameState.level * SPAWN_RATES.CUSTOMER_MIN_INTERVAL_DECREMENT;
    ...
    const disappointedEmoji = disappointedEmojis[Math.floor(Math.random() * disappointedEmojis.length)];
  ...
  }, [gameState.level, gameState.paused, lastCustomerSpawn]);
  ...
  const servePizza = useCallback(() => {
    ...
    setGameState((prev) => ({
      ...prev,
      pizzaSlices: [...prev.pizzaSlices, createPizzaSlice(gameState.chefLane)],
      availableSlices: prev.availableSlices - 1,
    }));
  }, [gameState.availableSlices, gameState.chefLane, gameState.gameOver, gameState.nyanSweep?.active, gameState.paused]);
  ...
  const moveChef = useCallback(
    ...
    [gameState.gameOver, gameState.nyanSweep?.active, gameState.paused],
  );
  ...
  const useOven = useCallback(() => {
    ...
    setGameState((prev) => {
      ...
      if (result.newState) return { ...prev, ...result.newState };
      return prev;
    });
  }, [gameState.chefLane, gameState.gameOver, gameState.paused]);
  ...
  const updateGame = useCallback(() => {
    setGameState((prev) => {
      ...
      customerUpdate.events.forEach((event) => {
        ...
        if (event === 'GAME_OVER' && newState.lives === 0) newState = triggerGameOver(newState, now);
      });
      ...
      newState.pizzaSlices.forEach((slice) => {
        ...
        newState.customers = newState.customers.map((customer) => {
          ...
          hitResult.events.forEach((event) => {
            ...
            if (event === 'UNFROZEN_AND_SERVED') {
              soundManager.customerUnfreeze();
              const baseScore = customer.critic ? SCORING.CUSTOMER_CRITIC : SCORING.CUSTOMER_NORMAL;
              customerScores.push(awardCustomer(newState, customer, baseScore, dogeMultiplier));
              gainLifeFromHappyCustomers(newState, dogeMultiplier);
              return;
            }
            ...
            if (event === 'WOOZY_STEP_2' || event === 'SERVED_NORMAL' || event === 'SERVED_CRITIC') {
              soundManager.customerServed();
              const baseScore = customer.critic ? SCORING.CUSTOMER_CRITIC : SCORING.CUSTOMER_NORMAL;
              customerScores.push(awardCustomer(newState, customer, baseScore, dogeMultiplier));

              if (customer.critic && event === 'SERVED_CRITIC') gainCriticLifeAtPosition(newState, customer, 50);
              else gainLifeFromHappyCustomers(newState, dogeMultiplier);
            }
          });
      ...
      if (hasStar && newState.availableSlices > 0) {
        newState.customers = newState.customers.map((customer) => {
          ...
          const baseScore = customer.critic ? SCORING.CUSTOMER_CRITIC : SCORING.CUSTOMER_NORMAL;
          starPowerScores.push(awardCustomer(newState, customer, baseScore, dogeMultiplier));

          if (!customer.critic) gainLifeFromHappyCustomers(newState, dogeMultiplier);
      ...
      newState.powerUps.forEach((powerUp) => {
        if (
          ...
          newState = applyCaughtPowerUpEffect(powerUp.type, newState, now, {
            hasDoge,
            dogeMultiplier,
            pushScore: (entry) => powerUpScores.push(entry),
          });
        }
      });

      newState.powerUps = newState.powerUps
        .filter((p) => !caughtPowerUpIds.has(p.id))
      ...
      newState.emptyPlates = newState.emptyPlates
        ...
        .filter((plate) => {
          if (plate.position <= 10 && plate.lane === newState.chefLane && !newState.nyanSweep?.active) {
            ...
            const pointsEarned = Math.floor(
              SCORING.PLATE_CAUGHT * dogeMultiplier * getStreakMultiplier(newState.stats.currentPlateStreak),
            );
      ...
      if (newState.nyanSweep?.active) {
        ...
        newState.customers = newState.customers.map((customer) => {
          ...
          if (!(isLaneHit && isPositionHit)) return customer;
          ...
          const baseScore = customer.critic ? SCORING.CUSTOMER_CRITIC : SCORING.CUSTOMER_NORMAL;
          nyanScores.push(awardCustomer(newState, customer, baseScore, dogeMultiplier));

          if (customer.critic) gainCriticLifeAtPosition(newState, customer, 55);
          else gainLifeFromHappyCustomers(newState, dogeMultiplier);
      ...
      if (targetLevel > newState.level) {
        ...
        const highestStoreLevel =
          Math.floor(targetLevel / GAME_CONFIG.STORE_LEVEL_INTERVAL) * GAME_CONFIG.STORE_LEVEL_INTERVAL;
        ...
        const crossedBossLevel = BOSS_CONFIG.TRIGGER_LEVELS.find((triggerLvl) => oldLevel < triggerLvl && targetLevel >= triggerLvl);

        if (
          crossedBossLevel !== undefined &&
          ...
          newState.pizzaSlices.forEach((slice) => {
            ...
            if (Math.abs(newState.bossBattle!.bossPosition - slice.position) < 10) {
              ...
              if (newState.bossBattle!.bossHealth <= 0) {
                ...
                const currentBossLevel = BOSS_CONFIG.TRIGGER_LEVELS.slice().reverse().find((lvl) => newState.level >= lvl);
  ...
  }, [addFloatingScore, applyCaughtPowerUpEffect, ovenSoundStates, triggerGameOver]);
  ...
  const buyPowerUp = useCallback((type: 'beer' | 'ice-cream' | 'honey') => {
    setGameState((prev) => {
      ...
      if (prev.bank >= powerUpCost) {
        ...
        const newPowerUp: PowerUp = createPowerUp({
  ...
  // left intact to avoid any behavior drift
    ...
    (type: PowerUpType) => {
      setGameState((prev) => {
        ...
        if (type === 'beer') {
          ...
          if (newState.lives === 0) newState = triggerGameOver(newState as GameState, now);
        } else if (type === 'star') {
          ...
          newState.activePowerUps = [...newState.activePowerUps.filter((p) => p.type !== type), { type, endTime: now + POWERUPS.DURATION }];
  ...
  useEffect(() => {
    ...
    if (!prevShowStore && currentShowStore) {
      setGameState((prev) => ({ ...prev, paused: true, ovens: calculateOvenPauseState(prev.ovens, true, now) }));
    }

    if (prevShowStore && !currentShowStore) {
      setGameState((prev) => ({ ...prev, paused: false, ovens: calculateOvenPauseState(prev.ovens, false, now) }));
    }
  ...
  }, [gameStarted, spawnCustomer, spawnPowerUp, updateGame]);
