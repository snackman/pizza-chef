...
const handleScoring = (state: GameState, basePoints: number, lane: number, position: number, isCustomerServe: boolean = false) => {
  ...
  state.score += points;
  state.bank += money;
  state.floatingScores.push({ id: `score-${Date.now()}-${Math.random()}`, points, lane, position, startTime: Date.now() });
...
const attemptLifeGain = (state: GameState, isCritic: boolean, position: number) => {
  ...
  const shouldGain = isCritic ? (position >= 50) : (state.happyCustomers % 8 === 0);

  if (shouldGain) {
    state.lives = Math.min(GAME_CONFIG.MAX_LIVES, state.lives + (hasDoge ? 2 : 1));
...
export const useGameLogic = (gameStarted: boolean = true) => {
  ...
  const spawnBossWave = useCallback((waveNum: number): BossMinion[] => {
    return Array.from({ length: BOSS_CONFIG.MINIONS_PER_WAVE }).map((_, i) => ({
      id: `minion-${Date.now()}-${waveNum}-${i}`, lane: i % 4, position: POSITIONS.SPAWN_X + (Math.floor(i / 4) * 15), speed: ENTITY_SPEEDS.MINION, defeated: false
    }));
  }, []);

  // --- Core Game Loop Logic Helpers (Mutates the draft state) ---
  ...
  const updateCustomerStatusEffects = (state: GameState, now: number) => {
    ...
    state.customers = state.customers.map(c => {
      if (c.served || c.leaving || c.badLuckBrian) return c;

      if (!hasHoney && c.hotHoneyAffected) c.hotHoneyAffected = false;
      if (!hasIceCream && c.frozen) { if (c.shouldBeFrozenByIceCream) c.frozen = false; }
  ...
  const handleCollisions = (state: GameState, now: number) => {
    ...
    state.pizzaSlices.forEach(slice => {
      ...
      state.customers = state.customers.map(c => {
        // Fix for "Double Feed": If served or leaving, ignore pizza
        if (consumed || c.served || c.disappointed || c.vomit || c.leaving) return c;
        ...
        if (c.woozy && (c.woozyState || 'normal') === 'normal' && !c.hotHoneyAffected && !c.frozen) {
           ...
           handleScoring(state, SCORING.CUSTOMER_FIRST_SLICE, c.lane, c.position, false);
           state.emptyPlates.push({ id: `plate-${now}-${c.id}-1`, lane: c.lane, position: c.position, speed: ENTITY_SPEEDS.PLATE });
           return { ...c, woozyState: 'drooling' };
        }
        ...
        handleScoring(state, basePoints, c.lane, c.position, true); 
    ...
    const sliceWentOffScreen = state.pizzaSlices.some(s => s.position >= POSITIONS.OFF_SCREEN_RIGHT);
    if (sliceWentOffScreen || powerUpHit) {
       state.stats.currentPlateStreak = 0;
    }
  };

  const updateCustomerPositions = (state: GameState, now: number) => {
    state.customers = state.customers.map(c => {
      if (c.frozen && !c.hotHoneyAffected) return c;

      let moveSpeed = c.speed;
      if (c.hotHoneyAffected) moveSpeed *= 0.5;
      // Because logic runs BEFORE movement, these states (served/nyan) are 
      // already set for the current frame, so the speed boost happens instantly.
      if (c.served || c.leaving || c.brianNyaned) moveSpeed *= 2; 
      if (c.brianNyaned) moveSpeed *= 1.5; 

      if (c.woozy) {
        if (c.movingRight) {
          const nextPos = c.position + (c.speed * 0.75);
          return nextPos >= POSITIONS.TURN_AROUND_POINT ? { ...c, position: nextPos, movingRight: false } : { ...c, position: nextPos };
        } else {
          const nextPos = c.position - (c.speed * 0.75);
  ...
  const updateGame = useCallback(() => {
    setGameState(prev => {
      ...
      // 3. Status Effects (Before movement)
      updateCustomerStatusEffects(newState, now);

      // 4. Move Projectiles (Before Collision)
      newState.pizzaSlices = newState.pizzaSlices.map(s => ({ ...s, position: s.position + s.speed })).filter(s => s.position < POSITIONS.OFF_SCREEN_RIGHT); 
      newState.powerUps = newState.powerUps.map(p => ({ ...p, position: p.position - p.speed })).filter(p => p.position > 10);
      
      // 5. Nyan Sweep (Correctly placed BEFORE movement to fix Nyan Brian bug)
         ...
         if (now - sweep.lastUpdateTime >= 50) {
            ...
            newState.customers = newState.customers.map(c => {
               if (!c.served && Math.abs(c.lane - newState.chefLane) < 1 && Math.abs(c.position - sweep.xPosition) < 10) {
                  ...
                  // Preserve specific Bad Luck Brian behavior
                  if (c.badLuckBrian) {
                     return { ...c, served: true, brianNyaned: true, leaving: true, movingRight: true, flipped: false };
                  }
      ...
      // 6. Handle Collisions (Correctly placed BEFORE movement to fix 1-Frame Lag)
      handleCollisions(newState, now);
      handleChefCollisions(newState, now);

      // 7. Move Customers (Correctly placed LAST)
      updateCustomerPositions(newState, now);
      
      // 8. Empty Plates (Catching Logic)
