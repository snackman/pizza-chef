...
/**
 * ==========================================
 * Refactor #1: Single source of truth for initial state
 * ==========================================
 */
const LANES = [0, 1, 2, 3] as const;

const createLaneRecord = <T,>(valueFactory: () => T) =>
  Object.fromEntries(LANES.map((l) => [l, valueFactory()])) as Record<number, T>;
...
const createZeroLaneMap = () => createLaneRecord(() => 0);
...
const createInitialGameState = (): GameState => ({
  ...
  availableSlices: 0,
  ovens: createOvens(),
  ovenUpgrades: createZeroLaneMap(),
  ovenSpeedUpgrades: createZeroLaneMap(),
  happyCustomers: 0,
  bank: 0,
  showStore: false,
...
/**
 * ==========================================
 * Refactor #2: Shared pause/resume oven handling
 * ==========================================
 */
const setPausedAndAdjustOvens = (prev: GameState, nextPaused: boolean, now: number): GameState => {
  const updatedOvens = { ...prev.ovens };

  Object.keys(updatedOvens).forEach((laneKey) => {
    const lane = parseInt(laneKey, 10);
  ...
  return { ...prev, paused: nextPaused, ovens: updatedOvens };
};
...
export const useGameLogic = (gameStarted: boolean = true) => {
  ...
  const togglePause = useCallback(() => {
    setGameState((prev) => setPausedAndAdjustOvens(prev, !prev.paused, Date.now()));
  }, []);
  ...
  useEffect(() => {
    ...
    if (!prevShowStore && currentShowStore) {
      setGameState((prev) => setPausedAndAdjustOvens(prev, true, Date.now()));
    }
    if (prevShowStore && !currentShowStore) {
      setGameState((prev) => setPausedAndAdjustOvens(prev, false, Date.now()));
    }
